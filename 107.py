# invoice_timesheets_compiler_fixed.py
import os
import time
import threading
import winsound
...
class App(tb.Window):
    def __init__(self):
        ...
        # TOP area: use THEME_BG for background (fixed)
        ...
        # use ttkbootstrap Checkbutton for nicer checkmark; ensure black label text if possible
        self.chk_vars = []
    ...
    def on_start(self):
        ...
        # basic validation: ensure numeric and in range
        ...
        # ensure latest scan results - wait short time
        if self.scan_thread and self.scan_thread.is_alive():
    ...
    def _run_merge(self, pre_scan_items, week_str):
        ...
        for client, info in pre_scan_items:
            try:
                ...
                # increment for each raw file processed
