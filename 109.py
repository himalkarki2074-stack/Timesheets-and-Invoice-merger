...
APP_TITLE = "Invoice and Timesheets Compiler"  # Updated title
THEME_BG  = "#2b6cb0"
PANEL_BG  = "#ffffff"
TEXT_FG   = "#000000"  # All text black
ACCENT    = "#1e90ff"
...
def collect_files_for_merge(folder):
    ...
    for name in sorted(os.listdir(folder)):
        ...
        if not os.path.isfile(path):
            continue
        lower, ext = name.lower(), os.path.splitext(name)[1].lower()
...
class App(tb.Window):
    def __init__(self):
        ...
        # Header
        tb.Label(outer, text=APP_TITLE, font=("Segoe UI", 20, "bold"),
                 bootstyle="light", foreground=TEXT_FG, background=PANEL_BG).pack(anchor="center", pady=(4, 12))
        ...
        # Clients panel
        clients_panel = tb.Labelframe(top_row, text="Clients", bootstyle="info", padding=10, foreground=TEXT_FG)
        ...
        # Date panel
        date_panel = tb.Labelframe(top_row, text="Week Ending (MM-DD)", bootstyle="info", padding=10, foreground=TEXT_FG)
        date_panel.pack(side="left", fill="both", expand=True)
        tb.Label(date_panel, text="Month (MM):", bootstyle="light", foreground=TEXT_FG).grid(row=0, column=0, padx=5, pady=2)
        self.month_var = tk.StringVar(value=datetime.now().strftime("%m"))
        self.month_entry = tb.Entry(date_panel, textvariable=self.month_var, width=6)
        self.month_entry.grid(row=0, column=1, padx=5, pady=2)

        tb.Label(date_panel, text="Day (DD):", bootstyle="light", foreground=TEXT_FG).grid(row=1, column=0, padx=5, pady=2)
        self.day_var = tk.StringVar(value=datetime.now().strftime("%d"))
        self.day_entry = tb.Entry(date_panel, textvariable=self.day_var, width=6)
        self.day_entry.grid(row=1, column=1, padx=5, pady=2)

        # Actions panel
        actions = tb.Labelframe(top_row, text="Actions", bootstyle="info", padding=10, foreground=TEXT_FG)
        ...
        # Progress
        prog_panel = tb.Labelframe(outer, text="Progress", bootstyle="info", padding=10, foreground=TEXT_FG)
        prog_panel.pack(fill="x", pady=(6,10))
        self.progress = tb.Progressbar(prog_panel, mode="determinate")
        self.progress.pack(fill="x")
        self.eta_label = tb.Label(prog_panel, text="0% • ETA: --", bootstyle="secondary", foreground=TEXT_FG)
        self.eta_label.pack(anchor="w", pady=(6,0))

        # Log panel
        log_panel = tb.Labelframe(outer, text="Log", bootstyle="info", padding=10, foreground=TEXT_FG)
        log_panel.pack(fill="both", expand=True)
        self.log_text = tk.Text(log_panel, height=16, wrap="word", bg=PANEL_BG, fg=TEXT_FG, relief="flat")
        ...
        # Footer
        tb.Label(outer, text=f"Root: {main_folder}", bootstyle="light", foreground=TEXT_FG).pack(anchor="w")
    ...
    def _run_merge(self, clients, week_str):
        ...
        for client in clients:
            try:
                ...
                if not week_path:
                    self.logger.log(f"Week folder not found: {client_root} -> 'Week {week_str}'", "warn")
                    continue
                ...
                out_name = f"{os.path.splitext(os.path.basename(invoice_pdf))[0]}_.pdf" if invoice_pdf else f"{client}_Week_{week_str}.pdf"
                out_path = os.path.join(week_path, out_name)
                merger.write(out_path)
                merger.close()
                self.logger.log(f"Merged PDF: {out_path}", "ok")
            ...
            remaining = int(elapsed / done * (total_tasks - done))
