# invoice_timesheets_compiler_blacktext.py
import os
import time
import threading
import winsound
...
class App(tb.Window):
    def __init__(self):
        ...
        # use tk.Checkbutton so we can set fg=TEXT_FG (black)
        self.chk_vars = []
        for c in clients_list:
            var = tk.BooleanVar(value=True)
            cb = tk.Checkbutton(clients_panel, text=c, variable=var, bg=PANEL_BG, fg=TEXT_FG, anchor="w",
                                selectcolor=ACCENT, relief="flat", font=("Segoe UI", 10))
            cb.pack(anchor="w", pady=2, padx=2, fill="x")
            self.chk_vars.append((c, var))

        # Select/None small buttons (styled colored, keep them colored)
        ...
        # Month & day (use ttk.Combobox and ensure black text)
        mrow = tb.Frame(date_panel)
        mrow.pack(anchor="w", pady=2, fill="x")
        lbl_month = tk.Label(mrow, text="Month (MM):", bg=PANEL_BG, fg=TEXT_FG)
        ...
        # style combobox foreground black
        try:
            self.month_combo.configure(foreground=TEXT_FG)
        except Exception:
            pass

        drow = tb.Frame(date_panel)
        drow.pack(anchor="w", pady=6, fill="x")
        lbl_day = tk.Label(drow, text="Day (DD):", bg=PANEL_BG, fg=TEXT_FG)
        ...
        try:
            self.day_combo.configure(foreground=TEXT_FG)
        except Exception:
            pass
        ...
        # Buttons: Start and Exit keep colored styles
        ...
        # set combobox foreground if possible
        try:
            self.client_select.configure(foreground=TEXT_FG)
        except Exception:
            pass

        self.queue_list = tk.Listbox(right_col, width=40, height=12, bg="white", fg=TEXT_FG)
        ...
        for client, info in pre_scan_items:
            try:
                ...
                if not week_path:
                    ...
                    self._increment_task_and_update(start_time)
                    continue
                else:
    ...
    def _increment_task_and_update(self, start_time):
        self.tasks_done += 1
